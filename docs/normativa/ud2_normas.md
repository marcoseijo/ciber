## 2.1 Normas de compliance UNE-ISO

A continuación se presentan las normas de compliance más relevantes, agrupadas por categoría:

---

### 🔍 Gestión general de compliance
- **UNE-ISO 37301:2021** – Sistema de gestión integral de compliance, guía de implementación y certificación.  
- **ISO 19600:2014** – Marco de referencia para sistemas de gestión de compliance (predecesora de UNE-ISO 37301, no certificable).

---

### ⚖️ Penal y tributaria
- **UNE 19601:2025** – Prevención de delitos corporativos de alto impacto.  
- **UNE 19602:2019** – Compliance tributario, asegura el cumplimiento fiscal de la empresa.

---

### 🚫 Antisoborno y prevención de fraudes
- **UNE-ISO 37001:2025** – Prevención de corrupción, cohecho y tráfico de influencias.  
- **UNE-ISO 37000** – Gobernanza de las organizaciones, incluyendo ética y transparencia.  
- **UNE-ISO 37002** – Sistemas de gestión de canales de denuncia.

---

### 🧩 Gestión de riesgos
- **UNE-ISO 31000:2018** – Marco internacional de gestión de riesgos aplicable a compliance corporativo.

---

### 🛡️ Seguridad de la información
- **ISO/IEC 27001:2022** – Gestión de seguridad de la información, soporte al compliance IT.  
- **ISO/IEC 27002:2022** – Código de buenas prácticas para controles de seguridad de la información.  
- **ISO/IEC 27005:2022** – Gestión de riesgos de seguridad de la información.

---

### 🔐 Privacidad y protección de datos
- **ISO/IEC 29100** – Marco de referencia para la privacidad de la información.  
- **ISO/IEC 29151** – Directrices para protección de datos personales.

---

## 2.2 Normativas obligatoria sobre seguridad, información y derechos digitales

Este apartado presenta una visión general del marco legal que regula la seguridad de la información, los servicios digitales, la propiedad intelectual y la protección de datos en España.

---

### 🛡️ Esquema Nacional de Seguridad (ENS)
El ENS establece un conjunto unificado de principios, requisitos y medidas para garantizar la protección de la información y los servicios electrónicos en la Administración Pública y sus proveedores.  
Asegura aspectos como confidencialidad, integridad, disponibilidad, trazabilidad y autenticidad.  
Ha evolucionado desde su creación en **2010**, con revisiones destacadas en **2015** y **2022**.

---

### 💻 Ley de Servicios de la Sociedad de la Información (LSSI)
Regula los aspectos jurídicos de los Servicios de la Sociedad de la Información cuando existe actividad económica.  
Incluye áreas como:

- Comercio electrónico  
- Contratación en línea  
- Información y publicidad  
- Servicios de intermediación  

---

### ✍️ Ley de Propiedad Intelectual (LPI)
Define los derechos de autor y derechos conexos que protegen las obras creativas (literarias, artísticas o científicas).  
Algunos puntos esenciales:

- La autoría nace automáticamente al crear la obra.  
- Los derechos morales del autor son **irrenunciables** e **intransferibles**.  
- La protección cubre cualquier formato o medio, presente o futuro.  
- Se incluyen patentes, marcas, diseños industriales y otros sistemas de protección.

---

### 🧩 Licencias Creative Commons (CC)
Las licencias **CC** permiten a los creadores especificar qué usos permiten de sus obras.  
Se aplican a textos, imágenes, vídeos, música y, en menor medida, software.  
Son útiles para decidir cómo compartir y reutilizar contenido respetando la autoría.

| Icono | Abreviatura | Significado | Descripción de la Condición |
| :---: | :---: | :---: | :--- |
| **👤** | **BY** | **Reconocimiento (Attribution)** | **Obligatorio en todas las licencias CC.** Debes dar crédito al autor original de forma adecuada, proporcionar un enlace a la licencia e indicar si se realizaron cambios. |
| **💲** | **NC** | **No Comercial (NonCommercial)** | No permite el uso de la obra para **fines lucrativos** o comerciales. |
| **🔁** | **SA** | **Compartir Igual (ShareAlike)** | Si remezclas, transformas o creas a partir del material, debes distribuir tu contribución bajo la **misma licencia** del original. |
| **🚫** | **ND** | **Sin Obra Derivada (NoDerivatives)** | No permite modificar, transformar o adaptar la obra. Solo se permite su uso en su **forma original**. |

---

### 🔐 Ley Orgánica de Protección de Datos y Garantía de los Derechos Digitales (LOPDGDD)
Normativa española alineada con el RGPD europeo. Regula el tratamiento de datos personales y garantiza los derechos digitales de la ciudadanía.  
Su estudio se desarrolla en detalle en la **Unidad Didáctica 4**.

---